...
function activate(context) {
    ...
    const saved = context.globalState.get('exportedSessions', {});
    exportedSessions = new Map(Object.entries(saved));
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('copilot-chat-export.exportChat', () => manualExportAll(context)), vscode.commands.registerCommand('copilot-chat-export.openChatsFolder', () => openChatsFolder()), vscode.commands.registerCommand('copilot-chat-export.exportCurrentSession', () => exportCurrentSession(context)), vscode.commands.registerCommand('copilot-chat-export.pushToGitHub', () => pushToGitHub()), vscode.commands.registerCommand('copilot-chat-export.gitStatus', () => showGitStatus()), statusBarItem, outputChannel);
...
function getVSCodeUserDataPath() {
    ...
    const customPath = config.get('vscodeDataPath', '').trim();
...
function debouncedExport(filePath, context) {
...
// ─── Parsing ────────────────────────────────────────────────────
function parseJsonSession(content) {
    try {
...
function parseJsonlSession(content) {
    try {
        ...
        if (first.kind !== 0 || !first.v) {
            return undefined;
        }
        const session = first.v;
...
function applyArrayUpdate(obj, keys, value) {
    let current = obj;
...
function getOutputDir() {
    ...
    const config = vscode.workspace.getConfiguration('copilotChatExport');
    const folderName = config.get('outputFolder', '.copilot-chats');
...
async function exportSessionFile(filePath, context) {
    ...
    try {
        ...
        if (exportConfig.get('addToGitignore', true)) {
            addToGitignore();
        }
        ...
        if (exportConfig.get('autoCommit', true)) {
...
async function scanAndExport(context) {
    ...
    if (exported > 0) {
        vscode.window.showInformationMessage(`Copilot Chat Export: Auto-exported ${exported} session(s).`, 'Open Folder').then(action => { if (action === 'Open Folder') {
            openChatsFolder();
        } });
    }
...
// ─── Commands ───────────────────────────────────────────────────
async function manualExportAll(context) {
    if (!getOutputDir()) {
...
async function exportCurrentSession(context) {
    const chatDirs = getChatSessionDirs();
    let newest;
    ...
    if (session?.sessionId) {
        exportedSessions.delete(session.sessionId);
    }
...
function addToGitignore() {
    ...
    const config = vscode.workspace.getConfiguration('copilotChatExport');
    const folder = config.get('outputFolder', '.copilot-chats');
...
// ─── Git Integration ────────────────────────────────────────────
const gitExecOpts = { encoding: 'utf-8', timeout: 30000 };
/**
 * Run a git command in the output directory. Returns stdout or undefined on error.
 */
function gitRun(outputDir, args) {
    try {
        const opts = { ...gitExecOpts, cwd: outputDir };
        return (0, child_process_1.execSync)(`git ${args}`, opts).trim();
    }
    catch (err) {
 ...
 */
function ensureGitRepo(outputDir) {
    const config = vscode.workspace.getConfiguration('copilotChatExport');
    const remoteUrl = config.get('gitRemoteUrl', '');
    if (!remoteUrl) {
        return false;
    }
    ...
    if (!fs.existsSync(gitDir)) {
        ...
        const branch = config.get('gitBranch', 'main');
 ...
 */
async function gitAutoCommit(outputDir, filename) {
    if (!ensureGitRepo(outputDir)) {
        return;
    }
...
async function pushToGitHub() {
    ...
    const remoteUrl = config.get('gitRemoteUrl', '');
    if (!remoteUrl) {
        const setNow = await vscode.window.showWarningMessage('No GitHub remote URL configured. Set it in settings (copilotChatExport.gitRemoteUrl).', 'Open Settings');
    ...
    await vscode.window.withProgress({
        ...
        if (result !== undefined) {
            ...
            vscode.window.showErrorMessage('Push failed. Check the "Copilot Chat Export" output channel for details.');
        }
    });
}
/**
...
async function showGitStatus() {
    ...
    const config = vscode.workspace.getConfiguration('copilotChatExport');
    const remoteUrl = config.get('gitRemoteUrl', '') || '(not configured)';
    const gitDir = path.join(outputDir, '.git');
    if (!fs.existsSync(gitDir)) {
        vscode.window.showInformationMessage(`Chat exports at: ${outputDir}\nGit: Not initialized\nRemote: ${remoteUrl}`);
        return;
    }
    ...
    outputChannel.appendLine('═══════════════════════════════\n');
    vscode.window.showInformationMessage(`${files.length} exports | Branch: ${branch} | Remote: ${remote}`, 'Push Now', 'Open Folder').then(action => {
        if (action === 'Push Now') {
            pushToGitHub();
        }
